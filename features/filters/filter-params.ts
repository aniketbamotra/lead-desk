...
import { defaultFilters, type Filters, type WebsiteFilterValue } from "./filters"
...
export function writeFilterParams(params: URLSearchParams, filters: Filters, vertical: VerticalConfig) {
  const keys = ["q", "state", "city", "website", "review", "stage", "min", "max", "due", ...vertical.filters.map((f) => `f.${f.id}`)]
