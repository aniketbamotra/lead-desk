-- Site condition: a person's assessment of a lead's website, with points
-- from config (domain/site-condition.ts). Kept separate from qual_score and
-- combined only when sorting.
--
-- The view lists its columns by name (it doesn't select l.*), so the new
-- columns are appended explicitly. Everything else, including the scoring,
-- is copied unchanged from 005_entity_only.sql, the view currently in the
-- database.
--
...
alter table public.leads
  ...
  add column if not exists site_condition_score integer;
...
with base as (
  select
    ...
    l.site_condition, l.site_condition_score
  from leads l
),
official_counts as (
