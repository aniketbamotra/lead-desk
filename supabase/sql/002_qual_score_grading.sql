-- Make qual_score rank leads within the crowded middle (344 leads at 20 and
-- 275 at 15 before this change) and fix two rules that misfired.
--
-- Changes from 001:
--   fixed  "management" penalty never matched (case-sensitive, names are caps)
--   fixed  area-code penalty hit every lead without a contact phone
--   graded owner titles: owner/dentist/member/partner... +15, office manager +8
--   graded NPPES recency: <=1y +10, <=2y +6, <=5y +3 (was <=2y +10)
--   new    single organisation under this official +5
--   new    registered in the last 3 years +8
-- Highest possible score is now 43 (was 30).
--
-- Same columns as 001, so CREATE OR REPLACE works. To undo, run section 2 of
-- 001_lead_queue_new_columns.sql again.
--
